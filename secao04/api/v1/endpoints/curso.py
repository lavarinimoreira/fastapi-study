...
async def post_curso(curso: CursoSchema, db: AsyncSession = Depends(get_session)):
    ...
    Args:
        curso (CursoSchema): Dados do curso recebidos no corpo da requisição.
        db (AsyncSession): Sessão assíncrona do banco de dados (injeção de dependência).

    Returns:
        CursoModel: Retorna o curso recém-criado.
    """

    # Cria um novo objeto do modelo CursoModel com os dados recebidos
    novo_curso = CursoModel(titulo=curso.titulo, aulas=curso.aulas, horas=curso.horas)

    # Adiciona o curso na sessão do banco de dados
    db.add(novo_curso)

    # Confirma a transação para persistir os dados no banco
    await db.commit()
